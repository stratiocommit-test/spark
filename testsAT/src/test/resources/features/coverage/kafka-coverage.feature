...
Feature: [Spark Kafka Coverage] Kafka Coverage tests
  ...
  Scenario:[Spark Kafka coverage][01] Deploy Kafka spark job
    ...
    When I send a 'POST' request to '/service/${SPARK_FW_NAME}/v1/submissions/create' based on 'schemas/SparkCoverage/kafka_curl.json' as 'json' with:
    ...
    Then I send a 'POST' request to '/service/${SPARK_FW_NAME}/v1/submissions/kill/!{driverKafka}' based on 'schemas/SparkCoverage/kafka_curl.json' as 'json' with:
      |   $.appResource  |  UPDATE  | http://spark-coverage.marathon.mesos:9000/jobs/kafka-${COVERAGE_VERSION}.jar | n/a     |
